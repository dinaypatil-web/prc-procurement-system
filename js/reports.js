...
/** Export to Excel via SheetJS with TreeView Grouping and Flat Sheets */
export function exportExcel(reportType = null, title = null) {
  ...
  const effectiveTitle = title || (state.viewLevel === 'material' ? 'Material Items Procurement Report' : 'PRC Procurement Report (Tree Hierarchy)');

  const wb = XLSX.utils.book_new();

  // 1. Tree Hierarchy Sheet (Grouped by PRC Number with nested Material Child Rows)
  const treeHeaders = [
    'Hierarchy Level',
    'PRC Number',
    'PRC Date',
    'Material Code',
    'Material Description / Job',
    'Department',
    'Priority',
    'Status',
    'Req Qty',
  ...
  const treeRows = [];
  prcs.forEach(p => {
    const pMaterials = p.materials || [];
    const age = getPRCAge(p);
    const pStatus = calculateStatus(p, pMaterials);

    // Parent PRC Header Row
    treeRows.push([
      `📁 PRC (${pMaterials.length} item${pMaterials.length === 1 ? '' : 's'})`,
      p.prNumber,
      fmtDate(p.prDate || p.createdAt),
      '',
      p.job || p.jobDesc || p.department || '',
      p.department || '',
      p.priority || 'Medium',
      pStatus,
      '',
      ...
      '',
      p.allocationNumber || '',
      fmtDate(p.allocationDate),
      p.buyerName || p.allocatedBy || '',
      p.rfqNumber || '',
      fmtDate(p.rfqDate),
      p.tcdNumber || '',
      fmtDate(p.tcdDate),
      p.poNumber || '',
      fmtDate(p.poDate),
      p.vendorName || p.vendor || '',
      fmtDate(p.deliveryDate),
      age
    ]);

    // Child Material Item Rows (Indented Tree branches)
    pMaterials.forEach((m, mIdx) => {
      const isLast = mIdx === pMaterials.length - 1;
      const branchSymbol = isLast ? '  └── 🔩 Material' : '  ├── 🔩 Material';
      const tot = parseFloat(m.quantity) || 0;
      const proc = parseFloat(m.processedQty) || 0;
      const cls = parseFloat(m.closedQty) || 0;
      const pend = Math.max(0, tot - proc - cls);
      const mStatus = m.status || calculateMaterialStatus(m, p);

      treeRows.push([
        branchSymbol,
        p.prNumber,
        fmtDate(m.prDate || p.prDate || p.createdAt),
        m.matCode || m.materialCode || '',
        m.description || '',
        p.department || '',
        p.priority || 'Medium',
        mStatus,
        ...
        pend,
        cls,
        m.allocationNumber || (p.materials?.length === 1 ? p.allocationNumber : '') || '',
        fmtDate(m.allocationDate || (p.materials?.length === 1 ? p.allocationDate : null)),
        m.buyerName || m.allocatedBy || (p.materials?.length === 1 ? (p.buyerName || p.allocatedBy) : '') || '',
        m.rfqNumber || (p.materials?.length === 1 ? p.rfqNumber : '') || '',
        fmtDate(m.rfqDate || (p.materials?.length === 1 ? p.rfqDate : null)),
        m.tcdNumber || (p.materials?.length === 1 ? p.tcdNumber : '') || '',
        fmtDate(m.tcdDate || (p.materials?.length === 1 ? p.tcdDate : null)),
        m.poNumber || (p.materials?.length === 1 ? p.poNumber : '') || '',
        fmtDate(m.poDate || (p.materials?.length === 1 ? p.poDate : null)),
        m.vendorName || m.vendor || (p.materials?.length === 1 ? (p.vendorName || p.vendor) : '') || '',
        fmtDate(m.deliveryDate || m.deliveryEndDate || (p.materials?.length === 1 ? p.deliveryDate : null)),
        age
      ]);
    });
  });

  const treeWS = XLSX.utils.aoa_to_sheet([treeHeaders, ...treeRows]);
  treeWS['!cols'] = treeHeaders.map(h => ({ wch: Math.max(h.length + 3, 16) }));
  XLSX.utils.book_append_sheet(wb, treeWS, 'PRC & Materials (Tree)');
  ...
  XLSX.utils.book_append_sheet(wb, prcWS, 'PRC Summary');

  // 3. Material Items Sheet
  ...
  // 4. Cover / Summary Sheet
  const coverData = [
    ['PRC Procurement System Report'],
  ...
  XLSX.utils.book_append_sheet(wb, coverWS, 'Report Info');

  const filename = `PRC_TreeReport_${new Date().toISOString().split('T')[0]}.xlsx`;
  XLSX.writeFile(wb, filename);
  toast(`Excel exported in TreeView style — ${prcs.length} PRCs, ${mats.length} materials`, 'success');
}
...
export async function exportPDF(title = null) {
  ...
  const primary = [0, 51, 102]; // Corporate Navy
  ...
  doc.text('PRC System — ' + reportTitle, 10, 14);
  doc.setFontSize(8.5);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text('PRC Procurement System — Confidential', 10, pageH - 5);
  }
