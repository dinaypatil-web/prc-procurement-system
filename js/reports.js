  ...
  prcs.forEach(p => {
    ...
    pMaterials.forEach((m, mIdx) => {
      ...
      treeRows.push([
        ...
        pend,
        cls,
        m.allocationNumber || p.allocationNumber || '',
        fmtDate(m.allocationDate || p.allocationDate),
        m.buyerName || m.allocatedBy || p.buyerName || p.allocatedBy || '',
        m.rfqNumber || p.rfqNumber || '',
        fmtDate(m.rfqDate || p.rfqDate),
        m.tcdNumber || p.tcdNumber || '',
        fmtDate(m.tcdDate || p.tcdDate),
        m.poNumber || p.poNumber || '',
        fmtDate(m.poDate || p.poDate),
        m.vendorName || m.vendor || p.vendorName || p.vendor || '',
        fmtDate(m.deliveryDate || m.deliveryEndDate || p.deliveryDate),
        age
      ]);
    });
  });
